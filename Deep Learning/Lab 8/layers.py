...
class Convolution(Layer):
    def __init__(self, x, filter):
        """
        :param x: Reference to the previous Layer
        :param filter: The filter this Convolutional Layer will use
        """
        assert(x.output.ndim == 3)
        assert(filter.output.ndim == 4)
        Layer.__init__(self, (filter.output.shape[0], x.output.shape[1]-filter.output.shape[2]+1, x.output.shape[2]-filter.output.shape[3]+1)) # n x h-s0+1 x w-s1+1
        self.x = x
        self.filter = filter
    ...
    def forward(self):
        ...
        Set this layer's output based on the outputs of the layers that feed into it.
        """
        self.output = conv2d.conv_filter_forward(self.x.output, self.filter.output)
    ...
    def backward(self):
        ...
        the input gradients into the gradients of the previous layers.
        """
        padding = self.filter.output.shape[2] - 1
        dF = conv2d.conv_expand_layers(self.x.output, self.grad)
        dx = conv2d.conv_filter_backward(self.grad, torch.flip(self.filter.output, [2,3]), padding=padding)
        dx = torch.squeeze(dx, 0)
        self.filter.accumulate_grad(dF)
        self.x.accumulate_grad(dx)
...
class Flatten(Layer):
    def __init__(self, x):
        ...
        """
        assert(x.output.ndim == 3)
        Layer.__init__(self, torch.numel(x.output))
        self.x = x
    ...
    def forward(self):
        ...
        Set this layer's output based on the outputs of the layers that feed into it.
        """
        self.output = torch.flatten(self.x.output)
    ...
    def backward(self):
        ...
        the input gradients into the gradients of the previous layers.
        """
        self.x.accumulate_grad(torch.reshape(self.grad, self.x.output.shape))
...
class SoftmaxLoss(Layer):
    ...
    def softmax(self, X):
        ...
        :return: The softmax of every value
        """
    ...
    def log_softmax(self, X):
        """
        :param X: Values to find the log of the softmax from
        :return: The log of the softmax of each value
        """
        scaled = X - torch.max(X)
        denom = torch.sum(torch.exp(scaled))
        return scaled - torch.log(denom)
    ...
    def forward(self):
        ...
        log_softmax = self.log_softmax(self.x.output) # ln(softmax).
        self.output = torch.sum(-self.y_true.output * log_softmax) # Cross-entropy loss
...
class L2Norm(Layer):
    ...
    def backward(self):
        ...
        the input gradients into the gradients of the previous layers.
        """
        dW = self.grad * self.W.output * self.reg_const # Scale the gradient of the regularized weights down a bit
        self.W.accumulate_grad(dW)
...
class Sum(Layer):
    def __init__(self, s1, s2):
        ...
        Layer.__init__(self, s1.output.shape)
        self.s1 = s1
        self.s2 = s2
